...
bool (*getLink)( EMAC_Device* pdev );
bool getLink_old( EMAC_Device* pdev );
bool getLink_demo( EMAC_Device* pdev );
    ...
    case IOC_INIT:
         {
          ...
          //------------------------------------
           ...
           getLink = &getLink_demo;
           if( demo_mode == 0 )
           {
                econf->MdioModeFlags |= MDIO_MODEFLG_LOOPBACK;
                getLink = &getLink_old;
           }
    ...
    case NET_LINK:
         {
           ...
           *(bool*)const_cast<void*>( arg ) = getLink( dctxdata );
         }
         break;
...
// !: -:
// ---------------------------------------------------------------------------
int32 drv_eth6412_plug ( const char* path, net_config& cfg, uint32 rxd_count, uint32 txd_count, uint32 flag )
{
  EMAC_Device pdev;
...
bool getLink_old( EMAC_Device* pdev )
{
    ...
    if( error == OSE_OK ) 
    {
        return ( sgop.value_gpio & 0x01 ) ? true : false;
    }
...
bool getLink_demo( EMAC_Device* pdev )
{
    return ( MDIO_RGET( LINK ) >> ((MDIO_Device*)pdev->hMDIO)->phyAddr ) & 0x01;
}
