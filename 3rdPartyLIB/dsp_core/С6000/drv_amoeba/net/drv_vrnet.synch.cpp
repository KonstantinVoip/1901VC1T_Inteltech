  ...
  switch( cmd )
  {
    ...
    case IOC_DEINIT:
    {
      ...
      if( dctx->data != NULL )
      {
        ...
        dctx->data = NULL;  // TODO: Free memory of dctxdata->rx and dctxdata->tx buffers!
      }
    }
    break;
    ...
    case NET_RECV_GET:
    {
      ...
      s_vrnet_ctxdata* dctxdata = (s_vrnet_ctxdata*)d->ctx->data;
      have_packet = vrnet_queue_have( dctxdata->rx_queue ) ? 1 : 0;
      synr = syn_data( &have_packet, sizeof(have_packet), 0 );
      if( ( synr != OSE_OK ) || ( have_packet == 0 ) )
      {
        (*carg) = NULL;
        return OSE_OK;
      }
    ...
    case NET_SEND_GET:
    {
      ...
      net_descriptor** carg = (net_descriptor**)const_cast<void*>( arg );
      s_vrnet_ctxdata* dctxdata = (s_vrnet_ctxdata*)d->ctx->data;
      have_packet = vrnet_queue_have( dctxdata->txf_queue ) ? 1 : 0;
      synr = syn_data( &have_packet, sizeof(have_packet), 0 );
      if( ( synr != OSE_OK ) || ( have_packet == 0 ) )
      {
        (*carg) = NULL;
    ...
    case NET_HOST_RECV_GET:
    {
      ...
      net_descriptor** carg = (net_descriptor**)const_cast<void*>( arg );
      s_vrnet_ctxdata* dctxdata = (s_vrnet_ctxdata*)d->ctx->data;
      have_packet = vrnet_queue_have( dctxdata->rxf_queue ) ? 1 : 0;
      synr = syn_data( &have_packet, sizeof(have_packet), 0 );
      if( ( synr != OSE_OK ) || ( have_packet == 0 ) )
      {
        (*carg) = NULL;
    ...
    case NET_HOST_RECV_PUT:
    {
      ...
      s_vrnet_ctxdata* dctxdata = (s_vrnet_ctxdata*)d->ctx->data;
      syn_begin();
      memcpy( &tcfg, &dctxdata->config, sizeof(net_config) );
      syn_end();
      if( netpp_parse( tcfg, (*carg) ) != NETPP_OK )
      {
    ...
    case NET_HOST_SEND_GET:
    {
      ...
      net_descriptor** carg = (net_descriptor**)const_cast<void*>( arg );
      s_vrnet_ctxdata* dctxdata = (s_vrnet_ctxdata*)d->ctx->data;
      have_packet = vrnet_queue_have( dctxdata->tx_queue ) ? 1 : 0;
      synr = syn_data( &have_packet, sizeof(have_packet), 0 );
      if( ( synr != OSE_OK ) || ( have_packet == 0 ) )
      {
        (*carg) = NULL;
    ...
    case NET_SET_CONFIG_CALLBACK:
    {
      ...
      s_vrnet_ctxdata* dctxdata = (s_vrnet_ctxdata*)d->ctx->data;
      syn_begin();
      if( dctxdata->callback )
      {
        syn_end();
        return OSE_ALLREADY_SET;
      }
