...
{
  int32 error;
  if(arg->device.device_address == hoot.address) return OSE_OK;
  sem_lock(sem_usbhc, SEM_INFINITY);
...
//-------------------------------------------------------------------------------------------------
void  ph_hc_write_bufport(register uint16* dst_usb, register uint8* src_mem, register int32 size_in_byte);
void  ph_hc_read_bufport(register uint8* dst_mem, register uint16* src_usb, register int32 size_in_byte);
//-------------------------------------------------------------------------------------------------
...
static int32 ph_hc_init()
{
  ...
  {
    hc_pipe[i].alloc = 0x0;
    memset(&hc_pipe[i].device, 0x0, sizeof(s_usbhc_config_device));
    memset(&hc_pipe[i].endpoint, 0x0, sizeof(s_usbhc_config_endpoint));  
  ...
  switch(tsk->irp.token)
  {
    ...
    case USB_IN_TOKEN:
    {
      ...
      else
      {
        if(REG_HCEP0_INTSTATUS & (BM_EP0IS_INDT | BM_EP0IS_SHORT | BM_EP0IS_NULL))
        {
          ...
          {
            ph_hc_read_bufport((uint8*)tsk->irp.data, &REG_HCEP0_BUFPORT, tsk->irp.size);
          }
        }
    ...
    case USB_OUT_TOKEN:
    {
      ...
      while(true)
      {
        ...
        ph_hc_write_bufport(&REG_HCEP0_BUFPORT, (uint8*)tsk->irp.data, tsk->irp.size);
...
int32 usbhc_bulk_transaction(s_usbhc_bulk* tsk)
{
  ...
  switch(tsk->irp.token)
  {
    case USB_OUT_TOKEN:
    {
      ...
      *pipe->reg.tbt = tsk->irp.size;
      *pipe->reg.control2 = BM_PIPE_ACTIVE;
      *pipe->reg.control1 &= ~BM_PIPE_AUTO;
      while(true)
      {
        ...
        REG_GPVAL |= 0x0040;
        ph_hc_write_bufport(pipe->reg.bufport, &((uint8*)tsk->irp.data)[tsk->irp.size-size_left], size);
        REG_GPVAL &= ~0x0040;

        if((tsk->irp.size & 0x1)==0x0) *pipe->reg.control2 |= BM_PIPE_DEND;
        else *pipe->reg.control2 |= BM_PIPE_DEND|BM_PIPE_BYTE_SET;
        
        usbhc_bulk_wait_interrupt(BM_PIPE1INT << npipe);

        if(*pipe->reg.intstatus & (BM_PIPEIS_HALT|BM_PIPEIS_STALL)) 
        {
          error = USB_ER_STALL;
          break;
        }
        else if(*pipe->reg.intstatus & BM_PIPEIS_OUTDT)
        {
          size_left -= size;
          *pipe->reg.intstatus = *pipe->reg.intstatus;
          continue;
        }
        else if(*pipe->reg.intstatus & BM_PIPEIS_COMPLETE) 
        {
          size_left -= size;
          error = 0x0;
          break;
    ...
    case USB_IN_TOKEN:
    {
      ...
      *pipe->reg.control2 = BM_PIPE_ACTIVE;
      ...
      while(true)
      {
        size = (size_left < tsk->irp.max_packet_length) ? size_left : tsk->irp.max_packet_length;
        //Data transfer / Data Stage complete <TR inactive>:
        if(*pipe->reg.intstatus & (BM_PIPEIS_HALT|BM_PIPEIS_STALL|BM_PIPEIS_ORUN)) 
        {
          error = USB_ER_STALL;
          break;
        }
        ...
        {
          if(*pipe->reg.intstatus & (BM_PIPEIS_INDT|BM_PIPEIS_SHORT|BM_PIPEIS_NULL))
          {
            //Reading received data:
            if(*pipe->reg.length == size)
            {
              ph_hc_read_bufport(&((uint8*)tsk->irp.data)[tsk->irp.size-size_left], pipe->reg.bufport, size);
              size_left -= size;
            }
          }
          if(*pipe->reg.intstatus & BM_PIPEIS_COMPLETE) 
          {
            error = 0x0;
            break;
          }
...
int32 ph_configuration_connect(s_usbhc_config* tsk)
{
  ...
  for(int32 i=0; i<tsk->number_endpoints; i++)
  {
    ...
    *pipe->reg.config1 &= ~BM_PIPE_EN;
    //Pipe configuration (Setting endpoint information):
    switch(pipe->config.endpoint->endpoint_number & 0x80) 
    {
      //Прием:
      case 0x80: *pipe->reg.config1 |= BM_BUFTYPE_W|BM_TR_TYPE_BLK|BM_DIR_IN|BM_PIPE_DIS; break;
      //Передача:
      case 0x00: *pipe->reg.config1 |= BM_BUFTYPE_W|BM_TR_TYPE_BLK|BM_DIR_OUT|BM_PIPE_DIS; break;
    }
    *pipe->reg.config1 |= (pipe->config.endpoint->endpoint_number & 0xf)<<4;
    *pipe->reg.config2  = (pipe->config.device_speed<<12)|pipe->config.endpoint->max_packet_size;
    *pipe->reg.devinfo  = pipe->config.device_address;
    //Enabling pipe:
    *pipe->reg.config1 |= BM_PIPE_EN;
    sleep_m(10);
    while( (*pipe->reg.intstatus & BM_PIPEIE_SM_CONF_E)==0 );
    *pipe->reg.intstatus |= BM_PIPEIE_SM_CONF_E;
    *pipe->reg.intenable  = BM_PIPEIE_SM_ERR_E|BM_PIPEIE_BYTE_E|BM_PIPEIE_HALT_E|BM_PIPEIE_NULL_E|BM_PIPEIE_ORUN_E|BM_PIPEIE_SHORT_E|BM_PIPEIE_SM_CONF_E|BM_PIPEIE_COMPLETE_E|BM_PIPEIE_OUTDT_E|BM_PIPEIE_STALL_E|BM_PIPEIE_INDT_E;
...
int32 ph_configuration_disconnect(s_usbhc_config* tsk)
{
  ...
  for(int32 i=0; i<tsk->number_endpoints; i++)
  {
    ...
    while( (*pipe->reg.status & BM_PIPEST_ACTIVE) != 0 );
    //Disabling pipe:
    *pipe->reg.config1 &= ~BM_PIPE_EN;
    //Confirmation of pipe unconfiguration
    while( (*pipe->reg.config1 & BM_PIPE_EN) != 0 );
    //Disabling pipe interrupts:
    REG_INT_ENABLE &= ~( BM_PIPE1INT_E << npipe );
    *pipe->reg.intenable  = 0x0;    
