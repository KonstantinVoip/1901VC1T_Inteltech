...
//                              ДРАЙВЕР GPIO
//-------------------------------------------------------------------------------------------------
// !: arch: tms320c64xx  для ЦП оборудования М-448-1.5
// !: sarc: all
//
// !: codepage: cp1251 
//
...
int32 gpio_reset_plis(s_gpio_load_plis* tsk)
{
    int32   gpio_prog, gpio_init, gpio_clk, gpio_dout, gpio_done;
    gpio_prog = 0x1<<tsk->gpio_prog;
    gpio_init = 0x1<<tsk->gpio_init;
    gpio_clk  = 0x1<<tsk->gpio_clk;
    gpio_dout = 0x1<<tsk->gpio_dout;
    gpio_done = 0x1<<tsk->gpio_done;
    ...
    REG_GPVAL &= ~(gpio_prog|gpio_clk|gpio_dout);
    //Write 0:
    REG_GPVAL &= ~gpio_prog;
    // Configure GPIO
    REG_GPEN &= ~( gpio_prog | gpio_init | gpio_clk | gpio_dout | gpio_done );
...
int32 gpio_load_plis( s_gpio_load_plis* tsk )
{
  register int32 error, gpio_prog, gpio_init, gpio_clk, gpio_dout, gpio_done;
...
//  uint32 bit, i;
  ...
  error = ( REG_GPVAL & gpio_done ) ? OSE_OK : OSE_FAIL;
