...
static uint32 syn_process_count = 0;
...
static uint32 syn_timeout = 30000000;//mk sec
static uint32 syn_sem_timeout = 60000;

// ---------------------------------------------------------------------------
static int32 modsyn_m448m15_error( int32 error )
{
  // Local definitions
  #define SOFTINT_COMM0   ( 1 << 10 )
  #define SOFTINT_COMM1   ( 1 << 9 )
  #define SOFTINT_ALLBITS ( SOFTINT_COMM0 | SOFTINT_COMM1 )
...
// ---------------------------------------------------------------------------
static bool modsyn_wait( uint32 address, uint8 b, uint64 tm_u )
{
  uint64 st = time_u();
  while( true )
  {
...
/*****************************************************************************
Syntax:   int32 modsyn_m448m15_init( int32 p_count, uint32 tmo_ms, void (*cbfunc)(int32) )   	    
Remarks:  mod_syn_init();			    
*******************************************************************************/
int32 modsyn_m448m15_init( int32 p_count, uint32 tmo_ms, void (*cbfunc)(int32) )
{
  int32 i, r;
  ...
  {
    memset((void*)0x6c000000,0x0,0x400);
  ...
  if( ( syn_interrupt = int_alloc( INT_GP4, &modsyn_interrupt ) ) == RES_VOID ) { r = OSE_CANT_CREATE_INTERRUPT; goto l_fail; }
  int_unlock( syn_interrupt );
...
// ---------------------------------------------------------------------------
int32 modsyn_m448m15_open( uint16 pid )
{
  int32 r;
  uint8 v;
  ...
  if( ( r = modsyn_m448m15_begin() ) != OSE_OK ) modsyn_fail( r );
  ...
  if( ( r = modsyn_m448m15_end() ) != OSE_OK ) modsyn_fail( r );
  
  return OSE_OK;
}
// ---------------------------------------------------------------------------
int32 modsyn_m448m15_close( uint16 pid )
{
  int32 r;
  uint8 v;
...
// ---------------------------------------------------------------------------
int32 modsyn_m448m15_time_s( uint32* tm_s )
{
  int32 r;
  uint8 v;
...
// ---------------------------------------------------------------------------
int32 modsyn_m448m15_time_m( uint64* tm_m )
{
  int32 r;
  uint8 v;
...
// ---------------------------------------------------------------------------
int32 modsyn_m448m15_time_u( uint64* tm_u )
{
  int32 r;
  uint8 v;
...
// ---------------------------------------------------------------------------
int32 modsyn_m448m15_time_core( uint64* tm_core )
{
  int32 r;
  uint8 v;
...
// ---------------------------------------------------------------------------
int32 modsyn_m448m15_read_osts( uint32* osts0_m0, uint32* osts1_m0, uint32* osts0_m1, uint32* osts1_m1 )
{
  uint8 v;
  register int32 r;
...
// ---------------------------------------------------------------------------
static int32 modsyn_m448m15_data0( void* data, uint32 length )
{
...
// ---------------------------------------------------------------------------
static int32 modsyn_m448m15_data1( void* data, uint32 length )
{
  register int32 r = OSE_OK;
  uint8 v;
...
// ---------------------------------------------------------------------------
int32 modsyn_m448m15_data( void* data, uint32 length, int32 prcn )
{
  register int32 r = OSE_OK;
  int32 tr;
  ...
  if( syn_processor == MAIN_0 )
  {
    if( prcn == MAIN_0 )
    {
      r = modsyn_m448m15_data0( data, length );
    }
    else if( prcn == MAIN_1 )
    {
      r = modsyn_m448m15_data1( data, length );
    }
  }
  else if( syn_processor == MAIN_1 )
  {
  ...
  if( ( tr = modsyn_m448m15_end() ) != OSE_OK ) modsyn_fail( tr );

  return r;
}
// ---------------------------------------------------------------------------
int32 modsyn_m448m15_wait_tm(uint64 timeout_us)//TIMEOUT V MKSEC
{
  (*reg_wait_txr) = 0x77;

  if( !modsyn_wait( off_wait_rxr, 0x77, timeout_us ) )
    modsyn_fail( OSE_SYN_TIMEOUT );
...
{
  { NULL, SYSCALL_SYN_INIT,       (uint32)&modsyn_m448m15_init      },
  { NULL, SYSCALL_SYN_OPEN,       (uint32)&modsyn_m448m15_open      },
  { NULL, SYSCALL_SYN_CLOSE,      (uint32)&modsyn_m448m15_close     },
  { NULL, SYSCALL_SYN_BEGIN,      (uint32)&modsyn_m448m15_begin     },
  { NULL, SYSCALL_SYN_END,        (uint32)&modsyn_m448m15_end       },
  { NULL, SYSCALL_SYN_TIME_S,     (uint32)&modsyn_m448m15_time_s    },
  { NULL, SYSCALL_SYN_TIME_M,     (uint32)&modsyn_m448m15_time_m    },
  { NULL, SYSCALL_SYN_TIME_U,     (uint32)&modsyn_m448m15_time_u    },
  { NULL, SYSCALL_SYN_TIME_CORE,  (uint32)&modsyn_m448m15_time_core },
  { NULL, SYSCALL_SYN_READ_OSTS,  (uint32)&modsyn_m448m15_read_osts },
  { NULL, SYSCALL_SYN_DATA,       (uint32)&modsyn_m448m15_data      },
  { NULL, SYSCALL_SYN_ERROR,      (uint32)&modsyn_m448m15_error     },
  { NULL, SYSCALL_SYN_WAIT_TM,    (uint32)&modsyn_m448m15_wait_tm   }
};
