...
#define RREG_FLAG0              0xc0001000
#define RREG_FLAG1              0xd0001000
#define RREG_DATA0              0xc0001008
#define RREG_DATA1              0xd0001008

#define WREG_FLAG               VREG64( 0xa0001000 )
#define WREG_DATA               VREG64( 0xa0001008 )
...
static void wait_plis()
{
  ...
  while( true )
  {
    uint64 rval0 = VREG64( 0xc0003ff8 );
    uint64 rval1 = VREG64( 0xd0003ff8 );
    if( mode & IPMP_DSP_M632_CH_FIRST ) c0 = ( rval0 == C_GOST2006 ) || ( rval0 == C_ALFA2006 ); else c0 = false;
    if( mode & IPMP_DSP_M632_CH_SECOND ) c1 = ( rval1 == C_GOST2006 ) || ( rval1 == C_ALFA2006 ); else c1 = false;
    if( !c0 && !c1 ) break;
    prc_yield();
  }
  // Wait for PLIS up
  while( true )
  {
    uint64 rval0 = VREG64( 0xc0003ff8 );
    uint64 rval1 = VREG64( 0xd0003ff8 );
    if( mode & IPMP_DSP_M632_CH_FIRST ) c0 = ( rval0 == C_GOST2006 ) || ( rval0 == C_ALFA2006 ); else c0 = true;
    if( mode & IPMP_DSP_M632_CH_SECOND ) c1 = ( rval1 == C_GOST2006 ) || ( rval1 == C_ALFA2006 ); else c1 = true;
    if( c0 && c1 ) break;
    prc_yield();
  }
}
...
static bool recv_8b( uint64* r )
{
  ...
  if( mode & IPMP_DSP_M632_CH_FIRST )
  {
  ...
  if( mode & IPMP_DSP_M632_CH_SECOND )
  {
...
// ---------------------------------------------------------------------------
static int ipmp_dsp_m632_tx_process( void* arg )
{
  register uint64* dp;
  uint64 t;
  uint32 i, l;
...
// ---------------------------------------------------------------------------
static int ipmp_dsp_m632_rx_process( void* arg )
{
  uint64 frame, d;
...
// ---------------------------------------------------------------------------
int32 ipmp_dsp_m632_comm_open( s_os_driver_descriptor* d, uint16 access )
{
  return -1;
}
// ---------------------------------------------------------------------------
int32 ipmp_dsp_m632_comm_close( s_os_driver_descriptor* d )
{
  return -1;
}
// ---------------------------------------------------------------------------
int32 ipmp_dsp_m632_comm_read( s_os_driver_descriptor* d, void* buf, int32 length )
{
  return -1;
}
// ---------------------------------------------------------------------------
int32 ipmp_dsp_m632_comm_write( s_os_driver_descriptor* d, const void* buf, int32 length )
{
  return -1;
}
//---------------------------------------------------------------------------
int32 ipmp_dsp_m632_comm_ioctl( s_os_driver_descriptor* d, int32 cmd, const void* arg )
{
  switch( cmd )
  {
    ...
    case IOC_INIT:
    {
      ...
      pattr.stack = 512;
      tx_process = prc_create( &ipmp_dsp_m632_tx_process, NULL, 0, &pattr );
      rx_process = prc_create( &ipmp_dsp_m632_rx_process, NULL, 0, &pattr );
...
// ---------------------------------------------------------------------------
int32 drv_ipmp_dsp_m632_plug( const char* path, uint32 s_mode )
{
  mode = s_mode;
  return drv_plug( path, INOT_FILE, &ipmp_dsp_m632_comm_open, &ipmp_dsp_m632_comm_close, &ipmp_dsp_m632_comm_read, &ipmp_dsp_m632_comm_write, &ipmp_dsp_m632_comm_ioctl, NULL, 0 );
}
// ---------------------------------------------------------------------------
int32 drv_ipmp_dsp_m632_unplug( const char* name )
{
